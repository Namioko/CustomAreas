...
var Tooltip_Tooltip = function Tooltip(_ref) {
  ...
  return title || content ? new jBox_all_default.a("Tooltip", {
    id: id,
    trigger: "mouseenter",
    attach: "#".concat(targetId),
    position: {
      x: "right",
      y: "top"
    },
...
var CommonFunctionsUtil = {
  ...
  getCoordinatesFromCircles: function getCoordinatesFromCircles(_ref) {
    var circles = _ref.circles;
    return circles.map(CommonFunctionsUtil.getCoordinatesFromCircle);
  }
};
/* harmony default export */ var src_CommonFunctionsUtil = (CommonFunctionsUtil);
...
var CustomArea_CustomArea = function CustomArea(_ref) {
  var _this = this;
  ...
  _defineProperty(this, "customizeToPolygon", function () {
    ...
    _this.polygon = _this.polygonWrapper.insert("polygon", "#".concat(circles[0].attr("id")));

    _this.polygon.attr("points", src_CommonFunctionsUtil.getCoordinatesFromCircles({
      circles: circles
    })).attr("id", "custom-area__polygon-".concat(index)).style("fill", _this.areaSettings.renderColors.fill).style("stroke", _this.areaSettings.renderColors.stroke).style("cursor", "pointer");
  });
  ...
  _defineProperty(this, "addDragHandlerToCircles", function () {
    _this.circles.forEach(function (circle, index) {
      ...
      if (index === 0) {
        _this.circleDragHandlerWithDeleteIcon(circle);
      } else {
        _this.circleDragHandler(circle);
      }
    });
  });
  ...
  _defineProperty(this, "addDeleteIcon", function () {
    ...
    _this.deleteIcon.attr("href", "./dist/bt-delete.png").attr("x", startPointPosition[0] - 15).attr("y", startPointPosition[1] - 15).attr("width", 15).attr("height", 15).attr("class", "deleteIcon").style("cursor", "pointer");
  ...
  _defineProperty(this, "initializeDragHandlers", function () {
    var handleCircleDrag = _this.handleCircleDrag,
        ...
        handleDrag = _this.handleDrag; // "function" (but not "=>") is used here to get d3 circle object which fires these events
  ...
  _defineProperty(this, "handleMouseOver", function () {
    ...
    if (hoverColors) {
      _this.polygon.style("fill", hoverColors.fill);

      _this.polygon.style("stroke", hoverColors.stroke);
    }
  });
...
var CustomAreasTool_CustomAreasTool = function CustomAreasTool(_ref) {
  ...
  CustomAreasTool_defineProperty(this, "initialize", function () {
    ...
    //     return false;
    // });
    _this.initializeImage();

    var areas = JSON.parse(sessionStorage.getItem("areas"));
  ...
  CustomAreasTool_defineProperty(this, "startDrawing", function () {
    if (!_this.state.drawing) {
      _this.state.drawing = true;

      _this.resetHelpObjects();
    }
  });

  CustomAreasTool_defineProperty(this, "addPoint", function (event) {
    var x, y;
    ...
    var circle = _this.activePolygonWrapper.append("circle");

    circle.attr("cx", x).attr("cy", y).attr("id", "custom-area__circle-".concat(_this.circles.length)).attr("r", 4).attr("fill", _this.circles.length === 0 ? "red" : "yellow").attr("stroke", "#000");

    if (_this.circles.length === 0) {
      circle.on("click", _this.generatePolygonForDesigner);
    }

    if (_this.circles.length === 2) {
      _this.circles[0].style("cursor", "pointer");
    }

    _this.circles.push(circle);

    _this.activeLine.attr("points", src_CommonFunctionsUtil.getCoordinatesFromCircles({
      circles: _this.circles
    }));
  });

  CustomAreasTool_defineProperty(this, "handleDrag", function (_ref2) {
    var isDragStarted = _ref2.isDragStarted;
    _this.state.dragging = isDragStarted;
  });

  CustomAreasTool_defineProperty(this, "checkIfDrawing", function () {
  ...
  CustomAreasTool_defineProperty(this, "generatePolygon", function (_ref3) {
    var area = _ref3.area;
    if (_this.circles.length < 3) return;

    _this.activeLine.remove();

    _this.state.drawing = false;

    _this.customAreas.push(new CustomArea_CustomArea({
      index: _this.state.lastPolygonIndex,
