...
var CommonFunctionsUtil = {
  ...
  getMiddleXPointFromPolygon: function getMiddleXPointFromPolygon(_ref2) {
    var polygonPoints = _ref2.polygonPoints;
    var xPoints = polygonPoints.filter(function (point, index) {
      return index % 2 === 0;
    });
    var intXPoints = xPoints.map(function (point) {
      return parseInt(point);
    });
    var result = intXPoints.reduce(function (result, point) {
      return result + point;
    }, 0);
    result /= intXPoints.length;
    result -= Math.min.apply(Math, _toConsumableArray(intXPoints));
    return result;
  }
};
